...
Work autonomously in the IsoAntiSAM repository and complete Phase 2. Read `phases/README.md` first and require a validated `PASS` handoff from Phase 1. This phase is empirical and numerical; GPU compute is restricted to small diagnostic scripts.

## 1. Objective
Empirically falsify or substantiate the theoretical predictions from Phase 1 regarding caustic collapse and sharp-needle entrapment. Construct high-dimensional synthetic non-convex loss surfaces containing both wide generalizable basins and narrow, spurious sharp needles. Measure trajectory dynamics, basin volume dilation, and transverse divergence scaling to prove that IsoAntiSAM escapes the caustic catastrophe that traps raw Anti-SAM.
...
1. **Synthetic Multi-Basin Test Landscape**:
   ...
   - Simulate parameter trajectories under standard Gradient Descent, Raw Anti-SAM ($\rho > 0$), and IsoAntiSAM ($\rho > 0$, isochoric gauge).
2. **Trajectory & Needle Entrapment Analysis**:
   ...
   - Verify whether IsoAntiSAM escapes the needle ($d \gg r_0$) and reaches the flat generalizable basin.
...
3. **Transverse Divergence Scaling Diagnostic**:
   ...
   - Verify the theoretical scaling law:
     ...
     - IsoAntiSAM: $\operatorname{div}_{T^\perp}(E_{\text{iso}}) = 0$ identically across all dimensions $d$.
...
- IsoAntiSAM escapes the sharp needle attractor ($d \gg r_0$) and converges to the flat minimum.
