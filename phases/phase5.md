...
3. **Equal-Budget Baseline Screening**:
   - Execute controlled, compute-matched benchmark runs across:
     ...
     - **Carve (Ours)** ($\rho = 0.05$, bilateral coherence gating, AdamW base).
...
4. **Generalization & Overfitting Diagnostics**:
   ...
   - Verify that Carve avoids the rapid validation divergence characteristic of Raw Anti-SAM.
...
- Training loss and validation loss descend synchronously under Carve without catastrophic divergence.
