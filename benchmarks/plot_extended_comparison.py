#!/usr/bin/env python3
"""
Generate publication-quality comparative diagnostic figures for Carve vs AdamW
...
log_dir = os.path.join(REPO_ROOT, 'logs/extended_study')
...
with open(os.path.join(log_dir, 'carve_30m_25ep_history.json'), 'r') as f:
    iso = json.load(f)
...
axes[0, 0].plot(epochs, iso['train_loss'], label='Carve Train', color='#1b9e77', lw=1.8, linestyle='--')
axes[0, 0].plot(epochs, iso['val_loss'], label='Carve Val', color='#1b9e77', lw=2.4, marker='s', ms=4)
...
axes[0, 1].plot(epochs, iso['val_ppl'], label='Carve (Ours)', color='#1b9e77', lw=2.4, marker='s', ms=4)
...
axes[1, 0].plot(epochs, iso['generalization_gap'], label='Carve Gap |L_val - L_train|', color='#1b9e77', lw=2.4, marker='s', ms=4)
...
axes[1, 1].plot(epochs, iso['lambda_max'], label='Carve $\\lambda_{\\max}$', color='#1b9e77', lw=2.4, marker='s', ms=4)
