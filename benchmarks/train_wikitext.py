...
Optimized with:
- FlashAttention / Scaled Dot-Product Attention (SDPA) hardware acceleration.
- Fused Bilateral Coherence Gating & Isochoric Gauging.
- Hessian spectral sharpness (lambda_max) power iteration diagnostics.
- Baseline comparisons: AdamW, Standard SAM, Raw Anti-SAM, IsoAntiSAM.
"""
...
class FlashCausalSelfAttention(nn.Module):
    """
    Hardware-accelerated Causal Self-Attention leveraging ROCm SDPA / FlashAttention.
    """
    ...
    def forward(self, x):
        ...
        # Use PyTorch's native FlashAttention SDPA kernel on MI300X
...
class CausalTransformer(nn.Module):
    def __init__(self, vocab_size=10000, dim=384, n_heads=6, n_layers=6, seq_len=256, dropout=0.1):
        ...
        self.lm_head.weight = self.token_emb.weight # Weight tying for ~14.6M params
...
# -----------------------------------------------------------------------------
def compute_hessian_max_eigenvalue(model, x, y, criterion, n_iters=5, r=1e-4):
    """
    Computes top eigenvalue of the Hessian matrix via Hessian-vector power iteration.
    Quantifies landscape sharpness / curvature without storing full O(d^2) Hessian.
    """
...
def main():
    ...
    if args.lr is None:
        if args.optimizer == 'sgd':
            args.lr = 0.05
        elif args.optimizer in ['sam', 'anti_sam']:
            args.lr = 5e-4
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Measure Hessian sharpness lambda_max on validation sample
        ...
        print(f"Epoch {epoch:2d}/{args.epochs:2d} | Train: {avg_train_loss:.4f} | Val: {avg_val_loss:.4f} | PPL: {val_ppl:6.2f} | λ_max: {lambda_max:6.2f} | Time: {epoch_dur:.2f}s")
    ...
    print(f"Final Val Loss: {history['val_loss'][-1]:.4f} | Final Val PPL: {history['val_ppl'][-1]:.2f} | Final λ_max: {history['lambda_max'][-1]:.2f}")
